...
class MultiAgentUnityWrapper(UnityToGymWrapper, MultiAgentEnv):
    ...
    # Override read-only properties from UnityToGymWrapper to allow setting
    @property
    ...
    def __init__(
        ...
        """
        self.unity_env = unity_env  # raw BaseEnv ONLY
        self._env = unity_env
...
def create_rllib_env(env_config: dict = {}):
    ...
    base_port = env_config.get("base_port", 50100)

    # Add a per-run random offset so restarts don't collide with dying processes
    run_offset = int(os.environ.get("SOCCER_PORT_OFFSET", "0"))
    base_port = base_port + run_offset

    env = soccer_twos.make(
        worker_id=worker_id,
        base_port=base_port,
        **{k: v for k, v in env_config.items()
           if k not in ("num_envs_per_worker", "worker_id", "base_port")}
    )

    if "multiagent" in env_config and not env_config["multiagent"]:
        return env

    print("RAW ENV TYPE:", type(env))

    # 🔥 ONLY wrap if NOT already wrapped
    if isinstance(env, MultiAgentUnityWrapper):
        return env  # already wrapped, do NOT wrap again


    print("Unwrapped to Unity BaseEnv:", type(env))  # should be UnityEnvironment

    return MultiAgentUnityWrapper(env)
