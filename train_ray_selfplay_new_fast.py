...
class CombinedCallback(DefaultCallbacks):
    ...
    def _try_advance_curriculum(self, trainer, iteration, mean_reward):
        ...
        #iteration on current task relative to task each task being a curriculum task to learn
        iters_on_task = iteration - self.task_start_iter
        #get the minimum threshold needed to advance
        threshold     = tasks[self.current_task].get("advance_threshold", 0.65)

        # Track consecutive successes; reset streak on any failure
        # use to determine if consistent enough Warning: Reseting may be too harsh, perhaps a steep penalty is better
        #or at least faster to train, could do consabove = max(0, self.consabove * 0.5 - 1)
